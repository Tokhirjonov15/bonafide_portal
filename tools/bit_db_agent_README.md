...
내원 횟수(2026-09-17): 등록 전송 때 그 차트번호의 `OcmInf` 접수 계열 행을 세어 `visits`(오늘 포함 N회째)·`prevVisit`(직전 내원일)·`firstVisit`(최초 내원일, `PbsNewDte` 가 없을 때) 로 보낸다. 환자 정보 창의 '지난 기록' 위에 "비트 기준 N회째 내원 · 최초 · 직전" 으로 표시. 차트번호는 SQL 매개변수로 넘긴다(가짜 차트 `Res_…` 포함).
전화(2026-09-17, 사용자 결정으로 추가 — 감시 스크립트 시절엔 읽지 않던 항목): `tel`=`PbsInf.PbsCelPhn`(없으면 `PbsPhnNum`) 숫자만. 카드의 전화 칸(문자 발송)과 진료 예약 상세에 쓰인다. 직원이 손으로 넣은 번호는 덮어쓰지 않는다.
...
비트 예약관리(`RsvInf`, 1진료실·16진료실, 하루 50~90건)를 오늘부터 `-ResvDays`(7)일치 읽어 동선관리의 **진료 예약** 화면(상단 메뉴 단추, 별도 모니터는 주소 뒤에 `?bitresv`)에 보여 준다. 4층 비급여 치료(시트) 화면과는 분리된 별도 페이지다.
