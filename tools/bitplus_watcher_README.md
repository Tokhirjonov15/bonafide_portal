...
`bitplus_watcher.ps1`(v3.2)은 세 채널을 합쳐 동선관리(Firebase `bitIntake`·`bitNote`)에 올립니다. 같은 스크립트를 접수 PC와 진료실 PC에 설치하며,
...
- 동선관리 상단 pill: 초록 = 정상, 노랑 = 비트 접수 창이 닫힘, 회색 = 12분 30초(750초) 이상 신호 없음(스크립트 꺼짐·PC 꺼짐). 하트비트는 5분마다.
