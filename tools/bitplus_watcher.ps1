...
# ── 비트플러스 접수 창 읽기 ──
function FindBit() {
  $p = Get-Process -Name BITRegistrations -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 } | Select-Object -First 1
  if ($p) { return $p.MainWindowHandle }; return [IntPtr]::Zero
}
...
while ($true) {
  try {
    if ($sentDay -ne (Today)) { $sent = @{}; $sentDay = (Today); TrimLog }
    $hwnd = FindBit
    $open = ($hwnd -ne [IntPtr]::Zero)
    # 하트비트
